...
import { ChallengeCategoryDB, insertChallengeRequestDto, newChallengeRequestDto, newChallengeResponseDto, selectTemplateRequestDto, selectTemplateResponseDto, writeChallengeResponseDto } from '../interfaces/writeDTO.js';
import { ErrorResponse, SuccessResponse } from '../modules/returnResponse.js';
import { TemplateDTO } from '../interfaces/DTO.js';
import { stream, logger } from '../config/logger.js';
 ...
 */
const newChallenge = async (req: Request<newChallengeRequestDto>, res: Response<newChallengeResponseDto>) => {
    try {
 ...
 *        
 */
const writeChallenge = async (req: Request, res: Response<writeChallengeResponseDto>) => {
    try {
 ...
 *          2. 서버 오류
 */
const selectTemplate = async (req: Request<selectTemplateRequestDto>, res: Response<selectTemplateResponseDto>) => {
    try {
 ...
 *            2. 완료(200)
 */
const insertTemporaryChallenge = async (req: Request<any, any, insertChallengeRequestDto>, res: Response) => {
    try {
 ...
 *            2. 완료(200)
 */
const insertChallengeComplete = async (req: Request<any, any, insertChallengeRequestDto>, res: Response) => {
    try {
        const { challengeName, challengeTitle, challengeContent } = req.body;
 ...
 *            2. 완료(200)
 */
const plannerTemporaryChallenge = async (req: Request<any, any, insertChallengeRequestDto>, res: Response) => {
    try {
        const { challengeName, challengeTitle, challengeContent } = req.body;
        const userId = req.decoded?.id;
...
export default {
    ...
    writeChallenge,
    selectTemplate,
    insertTemporaryChallenge,
    insertChallengeComplete,
    plannerTemporaryChallenge
